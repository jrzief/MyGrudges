...
class Application extends Component {
  ...
  componentDidMount() {
    ...
    API.get("mygrudgesCRUD", "/mygrudges").then(grudges => {
  ...
  addGrudge = grudge => {
    API.post("mygrudgesCRUD", "/mygrudges", { body: grudge }).then(() => {
      this.setState({ grudges: [grudge, ...this.state.grudges] });
    });
  };

  removeGrudge = grudge => {
    API.del("mygrudgesCRUD", "/mygrudges/object/" + grudge.id).then(() => {
      this.setState({
  ...
  toggle = grudge => {
    ...
    API.put("mygrudgesCRUD", "/mygrudges", { body: updatedGrudge }).then(() => {
